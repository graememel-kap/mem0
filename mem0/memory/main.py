...
# --------- helper utilities for OSS metadata filtering ---------
VECTOR_FILTER_KEYS = {"user_id", "agent_id", "run_id"}


def _split_vector_and_post_filters(
    filters: Optional[Dict[str, Any]],
) -> tuple[Dict[str, Any], Dict[str, Any]]:
    """Split incoming filters into those supported by vector stores and those to apply post-retrieval.

    Many OSS vector stores in mem0 only index a small set of top-level keys (user_id, agent_id, run_id).
    Arbitrary metadata (like type, session_id, message_index, etc.) is stored inside the payload and
    cannot be used for pre-filtering. To avoid runtime errors and still support metadata filtering,
    we split the filters and apply non-indexed keys after retrieval.

    Returns: (vector_filters, post_filters)
    """
    if not filters:
        return {}, {}
    vector_filters = {k: v for k, v in filters.items() if k in VECTOR_FILTER_KEYS}
    post_filters = {k: v for k, v in filters.items() if k not in VECTOR_FILTER_KEYS}
    return vector_filters, post_filters


def _apply_post_filters(
    items: list[Dict[str, Any]], post_filters: Dict[str, Any]
) -> list[Dict[str, Any]]:
    """Apply metadata-based filters on already formatted memory items.

    Each formatted item may have promoted keys (user_id, agent_id, run_id, actor_id, role) and a
    nested "metadata" dict containing additional payload fields. A post filter key matches either a
    top-level field or a key inside item["metadata"].

    Supports MongoDB-style operators:
    - Simple equality: {'key': 'value'}
    - $ne (not equal): {'key': {'$ne': 'value'}}
    - $gte (greater than or equal): {'key': {'$gte': 'value'}}
    - $lte (less than or equal): {'key': {'$lte': 'value'}}
    - $gt (greater than): {'key': {'$gt': 'value'}}
    - $lt (less than): {'key': {'$lt': 'value'}}
    - $in (in list): {'key': {'$in': ['value1', 'value2']}}
    - $nin (not in list): {'key': {'$nin': ['value1', 'value2']}}
    """
    if not post_filters:
        return items
    ...
    def _compare_values(actual, operator, expected):
        ...
        if operator == "$ne":
            ...
            # Handle both cases: actual value in expected list, or any item in actual list in expected list
            if isinstance(actual, list):
                return any(item in expected for item in actual)
            else:
                return actual in expected
        elif operator == "$nin":
            # Handle both cases: actual value not in expected list, or no item in actual list in expected list
            if isinstance(actual, list):
    ...
    def matches(item: Dict[str, Any]) -> bool:
        ...
        for k, expected in post_filters.items():
            # Prefer direct top-level value, else fall back to metadata blob
            actual = item.get(k, meta.get(k))

            if actual is None:
                # If the field doesn't exist, it doesn't match any filter
                return False

            # Handle MongoDB-style operators
            if isinstance(expected, dict) and any(
                key.startswith("$") for key in expected.keys()
            ):
                # This is a MongoDB-style operator filter
...
class Memory(MemoryBase):
    ...
    def add(
        ...
        *,
        ...
        # Only pass vector-store-supported filters to underlying vector operations
        vector_filters, _post_filters_unused = _split_vector_and_post_filters(
            effective_filters
        )
    ...
    def _get_all_from_vector_store(self, filters, limit):
        vector_filters, post_filters = _split_vector_and_post_filters(filters)
        memories_result = self.vector_store.list(filters=vector_filters, limit=limit)
        actual_memories = (
            memories_result[0]
        ...
        # Apply post-retrieval metadata filtering if any
        return _apply_post_filters(formatted_memories, post_filters)
    ...
    def _search_vector_store(
        ...
        memories = self.vector_store.search(
            query=query, vectors=embeddings, limit=limit, filters=vector_filters
        )
        ...
        # Apply post-retrieval metadata filtering and thresholding already handled
        return _apply_post_filters(original_memories, post_filters)
...
class AsyncMemory(MemoryBase):
    ...
    async def add(
        ...
        *,
        ...
        vector_store_task = asyncio.create_task(
            self._add_to_vector_store(
                messages, processed_metadata, vector_filters, infer
            )
        )
    ...
    async def _add_to_vector_store(
        ...
        async def process_fact_for_search(new_mem_content):
            ...
            existing_mems = await asyncio.to_thread(
                ...
                vectors=embeddings,
                limit=5,
                filters=effective_filters,  # pass only vector-supported filters
            )
            return [
    ...
    async def _get_all_from_vector_store(self, filters, limit):
        ...
        memories_result = await asyncio.to_thread(
            self.vector_store.list, filters=vector_filters, limit=limit
        )
        actual_memories = (
            memories_result[0]
            if isinstance(memories_result, (tuple, list)) and len(memories_result) > 0
